...
class RotatedRPNHead(AnchorHead):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            if 'weight_infrared' in img_meta:
                gt_bboxes_infrared_weights = img_meta['weight_infrared']
                # pos_gt_weightes = self.assign_weight(gt_bboxes_infrared_weights, gt_hbboxes,
                #                                      sampling_result.pos_gt_bboxes)
                pos_gt_weightes = gt_bboxes_infrared_weights[sampling_result.pos_assigned_gt_inds]
                pos_gt_weightes = pos_gt_weightes.unsqueeze(-1).repeat(1, 4).type(torch.float32)
                bbox_weights[pos_inds, :] = pos_gt_weightes.cuda()
            elif 'weight_rgb' in img_meta:
                gt_bboxes_rgb_weights = img_meta['weight_rgb']
                # pos_gt_weightes = self.assign_weight(gt_bboxes_rgb_weights, gt_hbboxes, sampling_result.pos_gt_bboxes)
                pos_gt_weightes = gt_bboxes_rgb_weights[sampling_result.pos_assigned_gt_inds]
                pos_gt_weightes = pos_gt_weightes.unsqueeze(-1).repeat(1, 4).type(torch.float32)
                bbox_weights[pos_inds, :] = pos_gt_weightes.cuda()
            else:
    ...
    def assign_weight(self, gt_weightes, gt_hbboxes, pos_gt_bboxes):
        # start = time.time()
        pos_gt_weightes = torch.zeros_like(pos_gt_bboxes)
        for i in range(gt_hbboxes.shape[0]):
            gt_hbbox = gt_hbboxes[i, :]
            for j in range(pos_gt_bboxes.shape[0]):
                pos_gt_bbox = pos_gt_bboxes[j, :]
                if all(gt_hbbox == pos_gt_bbox):
                    pos_gt_weightes[j, :] = gt_weightes[i]
        # end = time.time()
        # print("assign_weight time:", end - start)
        return pos_gt_weightes
