...
class ReResNet(BaseModule):
    ...
    def __init__(self,
        ...
        print("_make_stem_layer time:", end - start)
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        ...
        if not self.deep_stem:
            ...
            print("ennTrivialConv time:", end - start)
            start = time.time()
            self.norm1_name, norm1 = build_enn_norm_layer(
                stem_channels, postfix=1)
            end = time.time()
            print("build_enn_norm_layer time:", end - start)
            start = time.time()
            self.add_module(self.norm1_name, norm1)
            self.relu = ennReLU(stem_channels)
            end = time.time()
            print("ennReLU time:", end - start)
        start = time.time()
        self.maxpool = ennMaxPool(
            stem_channels, kernel_size=3, stride=2, padding=1)

        end = time.time()
        print("ennMaxPool time:", end - start)
