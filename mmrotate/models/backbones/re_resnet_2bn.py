...
class BasicBlock(enn.EquivariantModule):
    ...
    def __init__(self,
        ...
        self.norm1_name_, norm1_ = build_enn_norm_layer(
            self.mid_channels, postfix=str(1_1))

        self.norm2_name, norm2 = build_enn_norm_layer(out_channels, postfix=2)
        self.norm2_name_, norm2_ = build_enn_norm_layer(out_channels, postfix=str(2_1))
    ...
    def norm1_(self):
        """Get normalizion layer's name."""
        return getattr(self, self.norm1_name_)

    @property
    def norm2_(self):
        """Get normalizion layer's name."""
        return getattr(self, self.norm2_name_)

    def forward(self, x, RGB=False):
        """Forward function of BasicBlock."""

        def _inner_forward(x, RGB=False):
            if RGB:
                identity = x
        ...
        if self.with_cp and x.requires_grad:
            out = cp.checkpoint(_inner_forward, x, RGB=RGB)
        else:
            out = _inner_forward(x, RGB=RGB)
...
class Bottleneck(enn.EquivariantModule):
    ...
    def __init__(self,
        ...
        self.norm1_name_, norm1_ = build_enn_norm_layer(
            self.mid_channels, postfix=str(1_1))
        self.norm2_name_, norm2_ = build_enn_norm_layer(
            self.mid_channels, postfix=str(2_1))
        self.norm3_name_, norm3_ = build_enn_norm_layer(out_channels, postfix=str(3_1))
        ...
        self.add_module(self.norm3_name_, norm3_)
    ...
    def norm3_(self):
        """Get normalizion layer's name."""
        return getattr(self, self.norm3_name_)
...
class ReResNet_2BN(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ReResNet_2BN, self).__init__()
    ...
    def train(self, mode=True):
        ...
        super(ReResNet_2BN, self).train(mode)
        # self._freeze_stages()
