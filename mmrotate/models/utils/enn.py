...
def ennSoftmax(inplanes, dim=3):
    """enn Softmax."""
    in_type = build_enn_divide_feature(inplanes)
    return Softmax(in_type, dim=dim)
...
def ennAdaptiveAvgPool(inplanes, size=1):
    """enn Average Pooling.
    ...
    in_type = build_enn_divide_feature(inplanes)
    # pdb.set_trace()
    return enn.PointwiseAdaptiveAvgPool(in_type, size)


def ennAdaptiveMaxPool(inplanes, size=1):
    """enn Average Pooling.
    ...
    """
    in_type = build_enn_divide_feature(inplanes)
    return enn.PointwiseAdaptiveMaxPool(in_type, size)
...
def ennConcat(inplanes, dim=0):
    inplanes_list = []
    # pdb.set_trace()
    inplanes_list.append(inplanes[0].tensor)
    inplanes_list.append(inplanes[1].tensor)
    out = torch.cat(inplanes_list, dim)
    if dim == 1:
        out = enn.GeometricTensor(out, inplanes[0].type + inplanes[1].type)
    else:
        out = enn.GeometricTensor(out, inplanes[0].type)
    return out
