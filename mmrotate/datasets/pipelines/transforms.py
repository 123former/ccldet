...
@ROTATED_PIPELINES.register_module()
class PolyRandomRotate_Vis(object):
    """Rotate img & bbox.
    Reference: https://github.com/hukaixuan19970627/OrientedRepPoints_DOTA
    ...
    def __call__(self, results):
        ...
        img = self.apply_image(img, bound_h, bound_w)
        # img_path = r'/home/f523/guazai/disk3/shangxiping/mmrotate/vis.jpg'
        # mmcv.imwrite(img[:, :, :3], img_path)
        results['img'] = img
        results['img_shape'] = (bound_h, bound_w, c)
        gt_bboxes = results.get('gt_bboxes', [])
        labels = results.get('gt_labels', [])
        rgb_weight = results.get('gt_rgb_weight', [])
        gt_multi_num = results.get('gt_multi_num', [])
        ...
        gt_bboxes = gt_bboxes[keep_inds, :]
        labels = labels[keep_inds]
        rgb_weight = rgb_weight[keep_inds[int(gt_multi_num[0]):]]
        gt_multi_num = np.array([sum(keep_inds[:int(gt_multi_num[0])]), sum(keep_inds[int(gt_multi_num[0]):])])

        if len(gt_bboxes) == 0:
            return None
        results['gt_bboxes'] = gt_bboxes
        results['gt_labels'] = labels
        results['gt_rgb_weight'] = rgb_weight
        results['gt_multi_num'] = gt_multi_num
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if img.shape[2] == 6:
                ...
                results[key] = cv2.merge(img_list)
            else:
...
class Pad_Fusion:
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if results[key].shape[2] == 6:
                img = results[key]
                padded_img_list = []
                img_1 = img[..., 0:3]
                img_2 = img[..., 3:]
                if self.size is not None:
                    padded_img_1 = mmcv.impad(
                        img_1, shape=self.size, pad_val=pad_val)
                elif self.size_divisor is not None:
                    padded_img_1 = mmcv.impad_to_multiple(
                        img_1, self.size_divisor, pad_val=pad_val)
                padded_img_list.append(padded_img_1)

                if self.size is not None:
                    padded_img_2 = mmcv.impad(
                        img_2, shape=self.size, pad_val=pad_val)
                elif self.size_divisor is not None:
                    padded_img_2 = mmcv.impad_to_multiple(
                        img_2, self.size_divisor, pad_val=pad_val)
                padded_img_list.append(padded_img_2)

                padded_img = cv2.merge(padded_img_list)
            else:
                if self.size is not None:
